/* eslint-disable no-unused-vars */
import React from "react";
import logo from "../assets/logo.png";

const Navbar = () => {
  return (
    <div className="w-full bg-white  ">
    
      <nav className="px-4 md:px-12 flex items-center justify-between -ml-[100px] -mr-[80px]">
        <div className="flex items-center mb-5">
          <div className="text-[34px] font-[700] h-[41px] w-[185px] flex items-center">
            <span className="text-amber-500">
              <img src={logo} alt="" />
            </span>
            <span className="">Furniro</span>
          </div>
        </div>

        
        <div className="hidden md:flex items-center space-x-16 mb-5">
          <a href="#" className="hover:text-amber-500 md:text-[16px] font-[500]">
            Home
          </a>
          <a href="#" className="hover:text-amber-500 md:text-[16px] font-[500]">
            Shop
          </a>
          <a href="#" className="hover:text-amber-500 md:text-[16px] font-[500]">
            About
          </a>
          <a href="#" className="hover:text-amber-500 md:text-[16px] font-[500]">
            Contact
          </a>
        </div>

        <div className="flex items-center space-x-12 mb-5">
     
          <svg
            width="24"
            height="20"
            viewBox="0 0 24 20"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="w-6 h-6 cursor-pointer hover:text-amber-500"
          >
            <path
              d="M21.3333 10.0001V4.16675H23.6667V11.1667H21.3333M21.3333 15.8334H23.6667V13.5001H21.3333M9.66668 11.1667C12.7817 11.1667 19 12.7301 19 15.8334V19.3334H0.333344V15.8334C0.333344 12.7301 6.55168 11.1667 9.66668 11.1667ZM9.66668 0.666748C10.9044 0.666748 12.0913 1.15841 12.9665 2.03358C13.8417 2.90875 14.3333 4.09574 14.3333 5.33341C14.3333 6.57109 13.8417 7.75808 12.9665 8.63325C12.0913 9.50842 10.9044 10.0001 9.66668 10.0001C8.429 10.0001 7.24202 9.50842 6.36685 8.63325C5.49168 7.75808 5.00001 6.57109 5.00001 5.33341C5.00001 4.09574 5.49168 2.90875 6.36685 2.03358C7.24202 1.15841 8.429 0.666748 9.66668 0.666748ZM9.66668 13.3834C6.20168 13.3834 2.55001 15.0867 2.55001 15.8334V17.1167H16.7833V15.8334C16.7833 15.0867 13.1317 13.3834 9.66668 13.3834ZM9.66668 2.88341C9.0169 2.88341 8.39373 3.14154 7.93427 3.601C7.4748 4.06047 7.21668 4.68363 7.21668 5.33341C7.21668 5.98319 7.4748 6.60636 7.93427 7.06583C8.39373 7.52529 9.0169 7.78341 9.66668 7.78341C10.3165 7.78341 10.9396 7.52529 11.3991 7.06583C11.8586 6.60636 12.1167 5.98319 12.1167 5.33341C12.1167 4.68363 11.8586 4.06047 11.3991 3.601C10.9396 3.14154 10.3165 2.88341 9.66668 2.88341Z"
              fill="black"
            />
          </svg>

          <svg
            width="25"
            height="25"
            viewBox="0 0 25 25"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="w-6 h-6 cursor-pointer hover:text-amber-500"
          >
            <path
              d="M23.5 23.4999L18.2663 18.2569M21.1667 11.2499C21.1667 13.88 20.1219 16.4023 18.2622 18.2621C16.4024 20.1218 13.8801 21.1666 11.25 21.1666C8.61995 21.1666 6.0976 20.1218 4.23787 18.2621C2.37813 16.4023 1.33334 13.88 1.33334 11.2499C1.33334 8.61985 2.37813 6.09751 4.23787 4.23778C6.0976 2.37804 8.61995 1.33325 11.25 1.33325C13.8801 1.33325 16.4024 2.37804 18.2622 4.23778C20.1219 6.09751 21.1667 8.61985 21.1667 11.2499V11.2499Z"
              stroke="black"
              strokeWidth="2"
              strokeLinecap="round"
            />
          </svg>

          <svg
            width="26"
            height="24"
            viewBox="0 0 26 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="w-6 h-6 cursor-pointer hover:text-amber-500"
          >
            <path
              d="M7.16665 1.5C3.94548 1.5 1.33331 4.08533 1.33331 7.275C1.33331 9.84983 2.35415 15.9608 12.4026 22.1383C12.5826 22.2479 12.7893 22.3058 13 22.3058C13.2107 22.3058 13.4173 22.2479 13.5973 22.1383C23.6458 15.9608 24.6666 9.84983 24.6666 7.275C24.6666 4.08533 22.0545 1.5 18.8333 1.5C15.6121 1.5 13 5 13 5C13 5 10.3878 1.5 7.16665 1.5Z"
              stroke="black"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
          </svg>

          <svg
            width="26"
            height="23"
            viewBox="0 0 26 23"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="w-6 h-6 cursor-pointer hover:text-amber-500"
          >
            <path
              d="M24.2356 16.1926H7.95237L8.76995 14.5273L22.3543 14.5027C22.8137 14.5027 23.2074 14.1746 23.2895 13.7207L25.1707 3.19062C25.2199 2.91445 25.1461 2.63008 24.9656 2.41406C24.8765 2.30775 24.7652 2.22211 24.6396 2.16309C24.514 2.10407 24.3771 2.07308 24.2383 2.07227L6.95706 2.01484L6.8094 1.32031C6.71643 0.877344 6.31721 0.554688 5.86331 0.554688H1.6387C1.3827 0.554688 1.13719 0.656381 0.956172 0.837398C0.775156 1.01841 0.673462 1.26393 0.673462 1.51992C0.673462 1.77592 0.775156 2.02143 0.956172 2.20245C1.13719 2.38346 1.3827 2.48516 1.6387 2.48516H5.08127L5.72659 5.55312L7.31526 13.2449L5.26995 16.5836C5.16373 16.727 5.09975 16.8972 5.08526 17.075C5.07076 17.2528 5.10632 17.4312 5.18791 17.5898C5.35198 17.9152 5.68284 18.1203 6.04924 18.1203H7.76643C7.40035 18.6065 7.20261 19.1988 7.20315 19.8074C7.20315 21.3551 8.46096 22.6129 10.0086 22.6129C11.5563 22.6129 12.8141 21.3551 12.8141 19.8074C12.8141 19.1977 12.6117 18.6043 12.2508 18.1203H16.6559C16.2898 18.6065 16.0921 19.1988 16.0926 19.8074C16.0926 21.3551 17.3504 22.6129 18.8981 22.6129C20.4457 22.6129 21.7035 21.3551 21.7035 19.8074C21.7035 19.1977 21.5012 18.6043 21.1403 18.1203H24.2383C24.7688 18.1203 25.2035 17.6883 25.2035 17.1551C25.202 16.8994 25.0993 16.6546 24.9179 16.4743C24.7366 16.294 24.4913 16.1927 24.2356 16.1926ZM7.35901 3.91797L23.1035 3.96992L21.5613 12.6051L9.19377 12.627L7.35901 3.91797ZM10.0086 20.6715C9.53284 20.6715 9.14456 20.2832 9.14456 19.8074C9.14456 19.3316 9.53284 18.9434 10.0086 18.9434C10.4844 18.9434 10.8727 19.3316 10.8727 19.8074C10.8727 20.0366 10.7816 20.2564 10.6196 20.4184C10.4576 20.5805 10.2378 20.6715 10.0086 20.6715ZM18.8981 20.6715C18.4223 20.6715 18.034 20.2832 18.034 19.8074C18.034 19.3316 18.4223 18.9434 18.8981 18.9434C19.3739 18.9434 19.7621 19.3316 19.7621 19.8074C19.7621 20.0366 19.6711 20.2564 19.5091 20.4184C19.347 20.5805 19.1272 20.6715 18.8981 20.6715Z"
              fill="black"
            />
          </svg>
        </div>
      </nav>
    </div>
  );
};

export default Navbar;
